    ...
    public void runApplication() {
        ...
        while (true) {
            System.out.println("Введите ответ: 1 - Получить текущую погоду, " +
                    "2 - Получить погоду на следующие 5 дней, " + "3 - Получить данные предыдущих запросов, " +
                    "выход (exit) - завершить работу");
            String result = scanner.nextLine();

            checkIsExit(result);
    if(Integer.parseInt(result) == 3){   //если выбрали 3 пункт меню запускается метод вывода архива погоды в консоль но программа завершается
    try {
        Repository.getWeatherResponseData();
        break;
    }
