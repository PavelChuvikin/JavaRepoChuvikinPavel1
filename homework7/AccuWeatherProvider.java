...
public class AccuWeatherProvider implements WeatherProvider {
    ...
    public void getWeather(Periods periods) throws IOException {
        ...
        if (periods.equals(Periods.NOW)) {
            ...
            //Записываем ответ сервера в список
            List<WeatherResponse> weatherResponses = objectMapper.readValue(response.body().string().toLowerCase(), new TypeReference<List<WeatherResponse>>() {});
            //Записываем в toConsole текст для пользователя
            String toConsole = "Уважаемый пользователь, вы большой молодец! Температура воздуха: " + weatherResponses.get(0).gettemperature().getmetric().getvalue() +
            weatherResponses.get(0).gettemperature().getmetric().getunit();
